...
## Persistence 模块与策略初始化修复 (2026-02-08) 🔧✅

**问题概述**：
系统启动时发现多个初始化问题，导致策略无法正常加载和运行。

**根本原因**：
1. **Persistence 模块缺失**：`src/persistence/` 目录缺少 `__init__.py`，导致 Python 无法将其识别为包
2. **异步任务位置错误**：在 `__init__`（同步方法）中调用 `asyncio.create_task()`，可能导致事件循环未运行时失败
3. **属性初始化顺序错误**：创建 `OrderMonitor` 时使用了还未初始化的属性

**修复内容**：

1. **Persistence 模块修复**
   - 创建 `src/persistence/__init__.py`
   - 导出 `PersistenceAdapter` 和 `JsonPersistenceAdapter`
   - 使用 `Any` 类型替代具体的 `PersistenceAdapter` 类型，避免 Pylance 报错

2. **StateManager 异步任务修复**
   - 创建 `initialize_persistence()` 异步方法
   - 在 `engine.py` 的 `start()` 中，在事件循环运行后调用
   - 使用 `_needs_persistence_load` 标记延迟加载

3. **ScalperV2 属性初始化顺序修复**
   - 将 `self.enable_depth_protection`、`self.anti_flipping_threshold`、`self.tick_size` 等属性的初始化移到 `OrderMonitor` 创建之前
   - 删除重复的 `self.config` 初始化代码
   - 删除重复的 `self._last_compute_time` 等初始化代码

4. **engine.py 错误日志修复**
   - 在错误日志中添加 `exc_info=True`，显示完整堆栈信息
   - 便于精确定位错误发生位置

**修复效果**：
```python
修复前：
AttributeError: 'ScalperV2' object has no attribute 'enable_depth_protection'
Pylance: 无法解析导入 persistence.persistence_adapter

修复后：
✅ Persistence 模块可以被正确导入
✅ 异步任务在正确的时机初始化
✅ 所有属性在使用前已初始化
✅ 策略可以正常启动并运行
```
...
**修改的文件**：
1. `src/persistence/__init__.py` (新建)
2. `src/strategies/hft/components/state_manager.py` (修改)
3. `src/core/engine.py` (修改)
4. `src/strategies/hft/scalper_v2.py` (修改)

---

## 架构优化：工具函数、状态机与依赖注入 (2026-02-07) 🏗️✅

**优化概述**：
通过引入工具函数模块、状态机模式和依赖注入容器，系统架构更加清晰、可维护和可测试。

---

### 新增 1：工具函数模块 (2026-02-07) 🛠️✅
...
1. **PriceUtils (价格工具类)**
   - `format_price()`: 格式化价格显示（保留指定小数位）
   - `price_to_ticks()`: 价格转换为 tick 数量
   - `ticks_to_price()`: tick 数量转换为价格
   - `is_price_valid()`: 价格有效性检查
   - `round_to_tick()`: 价格按 tick 精度舍入

2. **TimeUtils (时间工具类)**
   - `format_duration()`: 格式化时间间隔（秒 → 可读字符串）
   - `is_trading_hours()`: 检查是否在交易时间
   - `get_market_open_time()`: 获取市场开盘时间

3. **PositionUtils (持仓工具类)**
   - `calculate_unrealized_pnl()`: 计算未实现盈亏
   - `calculate_pnl_pct()`: 计算盈亏百分比
   - `calculate_roe()`: 计算回报率
   - `format_position_size()`: 格式化持仓数量显示

4. **ValidationUtils (验证工具类)**
   - `is_valid_symbol()`: 交易对格式验证
   - `is_valid_order_size()`: 订单大小验证
   - `is_valid_price()`: 价格范围验证
   - `is_valid_amount()`: 金额验证

5. **MathUtils (数学工具类)**
   - `clamp()`: 数值限制在范围内
   - `lerp()`: 线性插值
   - `clamp_pnl()`: 盈亏百分比限制
   - `lerp_threshold()`: 阈值线性插值

6. **便捷函数**
   - `format_usdt()`: 格式化 USDT 金额
   - `format_price_with_side()`: 带交易方向的价格格式化
   - `calculate_position_size()`: 计算持仓大小（张数）
...
**使用示例**:
```python
from src.utils import PriceUtils, TimeUtils, format_usdt

# 价格格式化
price = 12345.6789
formatted = PriceUtils.format_price(price, tick_size=0.1)
# 结果: "12345.7"

# 时间格式化
seconds = 3661
formatted = TimeUtils.format_duration(seconds)
# 结果: "1h 1m 1s"

# 金额格式化
usdt = 1234.567
formatted = format_usdt(usdt)
# 结果: "1,234.57 USDT"
```

---

### 新增 2：状态机模式 (2026-02-07) 🔄✅

**文件**: `src/strategies/state_machine.py`

**核心功能**:

1. **StrategyState (状态枚举)**
   - `IDLE`: 空闲状态（无持仓，等待信号）
...
3. **StateMachine (状态机基类)**
   - `add_transition()`: 添加状态转换规则
   - `register_handler()`: 注册状态处理器
   - `update()`: 更新状态机（检查转换条件）
   - `handle_current_state()`: 处理当前状态
   - `force_transition()`: 强制转换到指定状态
   - `reset()`: 重置状态机
   - `get_state_info()`: 获取状态机信息
   - `has_transition()`: 检查是否存在指定转换

**设计特点**:
- ✅ 清晰的状态定义：使用枚举定义所有可能状态
- ✅ 灵活的转换规则：支持条件函数和转换动作
- ✅ 状态处理器：每个状态可注册独立的处理函数
- ✅ 易于调试：提供详细的状态转换日志
- ✅ 支持复杂状态流程

**使用示例**:
```python
from src.strategies.state_machine import StateMachine, StrategyState, StateTransition
...
1. **Container (依赖注入容器)**
   ...
   - `unregister()`: 注销服务
   - `clear()`: 清空所有服务
   - `get_all_services()`: 获取所有已注册的单例
   - `get_all_factories()`: 获取所有已注册的工厂
   - `info()`: 获取容器信息

2. **create_container()**: 便捷函数，创建容器实例

3. **@injectable**: 可注入装饰器，标记类可以被容器注入

**设计特点**:
- ✅ 简单易用：不引入复杂的 DI 框架
- ✅ 灵活性：支持单例和工厂函数两种注册方式
- ✅ 延迟加载：工厂函数按需创建实例
- ✅ 自动缓存：工厂函数创建的实例自动缓存为单例
- ✅ 易于测试：可以轻松 Mock 依赖
- ✅ 类型安全：完整的类型提示
...
class Engine:
    ...
    def _setup_container(self):
        ...
        self.container.register('order_manager', self.order_manager)
        self.container.register('position_manager', self.position_manager)

        # 注册策略工厂（延迟创建）
        self.container.register_factory('strategy', self._create_strategy)

    def _create_strategy(self, container):
        """策略工厂函数"""
...
**文档**: `docs/dependency_injection.md`
...
## 状态机与追单逻辑修复 (2026-02-08) 🔄✅

**问题概述**：
在代码审查和系统运行中发现状态机转换和追单逻辑存在多个关键问题，影响交易执行效率。

### 修复 1：撤单后无法追单

**问题描述**：
- `_cancel_maker_order()` 撤单成功后没有清除 `maker_order_id`
- 导致 `has_active_maker_order()` 仍然返回 True
- `_reorder_after_cancel()` 被开仓锁拦截，无法重新下单
- 追单功能完全失效

**根本原因**：
`_cancel_maker_order()` 撤单成功后只返回 True，但 `maker_order_id` 仍然存在。

**解决方案**：
在 `_cancel_maker_order()` 中撤单成功后立即清除 `maker_order_id` 并转换回 IDLE 状态：
...
**修复效果**：
```
修复前：撤单后 maker_order_id 仍然存在，重新下单失败
修复后：撤单后立即清除并转换回 IDLE，可以重新下单
```

### 修复 2：重复的状态转换

**问题描述**：
- `_place_maker_order()` 已经转换状态到 PENDING_OPEN
- `_handle_idle_state()` 又调用了一次 `_transition_to_state()`
- 导致重复转换，可能造成状态混乱

**解决方案**：
在 `_handle_idle_state()` 中移除重复的状态转换：

```python
if success:
    # 🔥 [修复] 状态转换已在 _place_maker_order() 中完成，避免重复转换
    logger.info(f"✅ [狙击挂单已提交] ...")
else:
...
撤单成功后：
  ...
  3. 返回 True
    ↓
调用 _reorder_after_cancel()
    ↓
状态是 IDLE，开仓锁已解锁 ✅
    ↓
调用 _place_maker_order()
    ↓
下单成功，转换到 PENDING_OPEN ✅
```
