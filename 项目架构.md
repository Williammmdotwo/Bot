...
### 10.11 WebSocket 网关"不坏金身"优化 (2026-01-27) ✅

**优化内容**: WebSocket 网关无限递归连接机制
...
- **测试脚本**: `test_ws_stability.py`
- **文档**: `docs/WEBSOCKET_IMMORTALITY.md`

---

### 10.12 ScalperV1 入场撮合逻辑优化 (2026-01-27) ✅
...
### 10.13 ScalperV1 测试套件修复 (2026-01-24) 🔥
...
### 10.12 测试套件全面修复 (2026-01-27) 🔥

**修复40：PositionManager 异步方法调用修复** ✅
- **问题描述**：测试代码同步调用了改为异步的方法，导致 "coroutine was never awaited" 警告和断言失败
- **影响测试**（9个）：
  - test_ghost_position_cleanup_sync_from_rest
  - test_ghost_position_cleanup_from_order
  - test_cancel_failure_with_status_check
  - test_update_position_from_api
  - test_update_position_short
  - test_update_position_zero
  - test_update_position_from_order_buy
  - test_update_position_from_order_sell_close
  - test_update_position_from_order_sell_close_all
- **修复方案**：为以下方法调用添加 `await`：
  - `_update_position` (8处)
  - `_update_position_from_order` (3处)
  - `update_from_event` (1处)
- **效果**：PositionManager 测试全部通过，无警告

**修复41：conftest.py Mock 配置增强** ✅
- **问题1：get_instrument_details 方法缺失**
  - Mock 返回 coroutine 对象，导致 TypeError
  - **修复**：添加正确的 mock 方法，返回合理的合约面值
    - DOGE-USDT-SWAP: ctVal=10.0
    - BTC-USDT-SWAP: ctVal=100.0
    - 其他: ctVal=1.0（默认）
...
- **计算公式**：`size * price * ctVal = 0.5 * 100 * 10 = 500 USDT`
- **影响测试**（4个）：
  - test_ema_bullish_trend_allows_entry
  - test_tight_spread_allows_entry
  - test_v2_standard_entry_all_conditions_met
  - test_cooldown_logic_allows_entry
...
### 测试覆盖率总结
- ✅ OrderManager: 11个测试
  - 关键修复：市价单安全、止损价格持久化、clOrdId 查找
  - 基本功能：下单、撤单、风控、查询
- ✅ PositionManager: 20个测试
  - 关键修复：幽灵单清理、撤单失败处理、指数退避/熔断
  - 基本功能：持仓更新、盈亏计算、对账、重置
- ✅ ScalperV1: 37个测试
  - V2 趋势过滤（EMA）
  - V2 价差过滤
  - V2 追踪止损
