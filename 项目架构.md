...
*项目目前运行在新加坡服务器1G1CPU*
*最后更新：2026年1月15日 - 添加双向仓位网格策略规划*

---

## 🎯 下一阶段目标：双向仓位网格策略 (Hedge Mode Grid Strategy)

### 策略概述

**HedgeModeGrid** 是一个基于价格区间的网格策略，允许同时持有多头和空头仓位，在横盘震荡市场中自动低买高卖。

#### 核心逻辑
1. **网格设置**：在价格区间内预设多层限价单（买低卖高）
2. **Hedge Mode**：OKX 允许同时持有多头和空头
3. **自动补单**：成交后自动在对应价格挂新单
4. **动态调整**：根据ATR调整网格间距

#### 策略特点
- **适合市场**：横盘震荡（BTC/ETH 70% 时间）
- **风险等级**：中等（需要保证金管理）
- **持仓模式**：Hedge Mode（双向持仓）
- **订单类型**：限价单（Maker 模式）
- **依赖**：无 pandas/numpy（原生 Python）
...
```python
@dataclass
class HedgeModeGridConfig:
    """网格策略配置"""
    symbol: str = "BTC-USDT-SWAP"

    # 网格参数
    grid_levels: int = 10            # 网格层数
    grid_spacing_pct: float = 0.005   # 网格间距（0.5%）
    price_range_high: float = 50000.0  # 价格区间上限
    price_range_low: float = 40000.0   # 价格区间下限

    # 仓位管理
    base_position_size: float = 0.01   # 每层仓位大小
    max_total_size: float = 0.1       # 最大总持仓
    max_long_size: float = 0.05       # 最大多头持仓
    max_short_size: float = 0.05       # 最大空头持仓

    # 动态调整
    dynamic_grid: bool = True          # 是否启用动态网格
    atr_period: int = 14             # ATR周期
    atr_multiplier: float = 1.5       # ATR倍数（网格间距）

    # 风控参数
    max_drawdown_pct: float = 0.10    # 最大回撤 10%
    stop_loss_pct: float = 0.05        # 硬止损 5%
...
```python
class HedgeModeGrid(BaseStrategy):
...
```python
RiskProfile(
    strategy_id="hedge_mode_grid",
    max_leverage=2.0,                    # 允许 2 倍杠杆
    stop_loss_type=StopLossType.HARD_PRICE, # 硬价格止损
    single_loss_cap_pct=0.02,             # 单笔最大亏损 2%
    max_order_size_usdt=1000.0,           # 单笔订单金额
    max_daily_loss_pct=0.05                 # 每日最大亏损 5%
)
```

#### 多层风控
1. **PreTradeCheck**：单笔订单金额和频率
2. **RiskProfile**：策略级杠杆和止损
3. **CapitalCommander**：资金分配和购买力
4. **PositionManager**：持仓对账和幽灵单防护
5. **网格专用风控**：
   - 最大总持仓限制（防止仓位过大）
   - 最大多头/空头持仓限制（防止单边暴露）
   - 价格区间监控（超出范围重置）
...
#### 阶段一：基础框架（核心功能）
- [ ] 创建策略文件 `src/strategies/grid/hedge_mode_grid.py`
- [ ] 实现 `HedgeModeGridConfig` 和 `GridLevel` 数据类
- [ ] 实现网格初始化逻辑（`initialize_grid()`）
- [ ] 实现订单成交处理（`on_order_filled()`）
- [ ] 实现自动补单逻辑（成交后挂新单）
- [ ] 添加基础日志和统计信息
...
```bash
# Hedge Mode Grid 配置
ACTIVE_STRATEGY=hedge_mode_grid
GRID_SYMBOL=BTC-USDT-SWAP
GRID_CAPITAL=10000.0

# 网格参数
GRID_LEVELS=10
GRID_SPACING_PCT=0.005
GRID_PRICE_HIGH=50000.0
GRID_PRICE_LOW=40000.0

# 仓位管理
GRID_BASE_SIZE=0.01
GRID_MAX_TOTAL_SIZE=0.1
GRID_MAX_LONG_SIZE=0.05
GRID_MAX_SHORT_SIZE=0.05
...
#### 1. OKX Hedge Mode 支持
**挑战**：OKX V5 API 需要正确设置 `posSide` 参数
- 多头持仓：`posSide="long"`
- 空头持仓：`posSide="short"`

**解决方案**：
- 在 OrderManager 中增强 `place_order()` 方法
- 支持显式传递 `posSide` 参数
- 确保 Hedge Mode 持仓正确标记

#### 2. 网格管理复杂性
**挑战**：多层订单同时存在，状态管理复杂

**解决方案**：
- 使用 `GridLevel` 数据结构追踪每层状态
- 订单 ID 到网格层级的映射
- 使用本地状态（类似 ScalperV1 的 `local_pos_size`）

#### 3. 原生 Python ATR 计算
**挑战**：不使用 pandas/numpy，需要手写 ATR

**解决方案**：
- 参考现有的 `VolatilityEstimator` 实现
- 使用 EMA 平滑 True Range
- 维护固定长度的价格历史（最多 14 个）

#### 4. 持仓对账
**挑战**：Hedge Mode 持仓计算复杂（多头+空头）

**解决方案**：
- 利用现有的 `PositionManager`（已支持 `signed_size`）
- 网格策略本地维护期望持仓
- 通过 Shadow Ledger 对账机制同步

### 与现有策略对比

| 特性 | ScalperV1 (HFT) | HedgeModeGrid (网格) |
|------|-------------------|---------------------|
| 频率 | 极高（秒级） | 中低（分钟/小时级） |
| 市场环境 | 波动/趋势 | 横盘震荡 |
| 持仓模式 | 单向 | 双向（Hedge Mode） |
| 订单类型 | 限价+市价 | 限价单为主 |
| 风险等级 | 高（快进快出） | 中等（需保证金管理） |
| 资金占用 | 低 | 中（双边持仓） |
| 适合场景 | 超短线剥头皮 | 区间套利 |
