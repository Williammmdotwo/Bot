...
*项目目前运行在新加坡服务器1G1CPU*
*最后更新：2026年1月14日 - 移除SniperStrategy和VultureStrategy，简化策略架构*
...
- 动量触发：当1秒内买入量 > 卖出量 * 3 且买入量 > 阈值时，立即市价买入
- 光速离场：
  - 止盈：+0.2% 立即走人
...
**⭐ 2026年1月13日重大修复**：
1. **Tick 过滤逻辑修复**：
   - **修复前**：只检查买入量 `if self.buy_vol < min_flow: return`
   - **修复后**：检查总成交量 `total_vol = buy_vol + sell_vol`
   - **效果**：不再丢失市场流动性数据，大单拆分后仍能正确触发

2. **下单逻辑优化**：
   - **数量强制整数**：`trade_size = max(1, int(size))`
   - **价格 Taker 吃单**：`entry_price = tick.price * 1.001` (溢价 0.1%)
   - **效果**：避免 OKX API 错误 59002 (sz 必须字符串) 和 51006 (价格精度)

3. **本地持仓记录**（新增）：
   - `self.local_pos_size = 0.0` - 本地强持仓记录
   - 开仓时立即记录：`self.local_pos_size = float(trade_size)`
   - 平仓时使用：`size=self.local_pos_size`
   - 平仓后重置：`self.local_pos_size = 0.0`
   - **效果**：不依赖 PositionManager 的实时性，解决 WebSocket 延迟问题
