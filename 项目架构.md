...
- **问题根因**：
  ...
  3. 但实际提取逻辑不完整或不存在
- **修复方案**：在 on_tick 方法最开始定义 `current_price` 变量
- **关键逻辑**：
  1. 从 tick 数据中提取：`bid`, `ask`, `last_price`
  2. 定义通用 `current_price`：
     - 优先使用 `last_price`
     - 如果 `last_price <= 0`，使用 `(bid + ask) / 2.0`
     - 如果没有数据，使用 `0.0`
  3. 所有使用 `price` 的地方改为使用 `current_price`
  4. 修改 4 个方法调用：
