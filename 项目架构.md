...
*项目目前运行在新加坡服务器1G1CPU*
*最后更新：2026年1月13日 - ScalperV1全面修复，OKX API参数格式优化*
...
- `stop()`: 停止策略，断开网关，优雅退出
- `_load_strategy()`: 动态加载策略（支持vulture, sniper, scalper_v1）
...
#### 3.4.3 VultureStrategy (秃鹫策略)
**文件**：hft/vulture.py

**策略逻辑**：
- 闪崩接针策略
- 在价格偏离EMA时发送IOC限价单抢反弹
- 使用原生Python实现EMA计算（无pandas依赖）

**风控配置**：
```python
RiskProfile(
    strategy_id="vulture",
    max_leverage=5.0,  # 允许5倍杠杆
    stop_loss_type=StopLossType.HARD_PRICE,
    max_order_size_usdt=500.0
)
```

#### 3.4.4 ScalperV1 (极速剥头皮策略) ⭐ NEW
...
#### 3.4.5 DualEMAStrategy (双均线趋势策略)
**文件**：trend/dual_ema.py

**策略逻辑**：
- 使用9周期EMA和21周期EMA的交叉信号生成交易信号
- 金叉：快线从下往上穿过慢线 → 买入信号
- 死叉：快线从上往下穿过慢线 → 卖出信号
- **已重构**：完全移除pandas依赖，使用原生Python
...
```
1. K线合成（简化版）
   - 维护 closes 列表（最多100个数据点）
   - 每隔 timeframe_minutes 分钟更新一次收盘价

2. 计算EMA（原生Python实现）
   - current_ema_fast = self._calculate_ema(closes, 9)
   - current_ema_slow = self._calculate_ema(closes, 21)

3. 检测交叉
   - 金叉: current_fast > current_slow and prev_fast <= prev_slow
   - 死叉: current_fast < current_slow and prev_fast >= prev_slow
...
EventBus (core/event_bus.py)
    ...
    └→ Strategies
        ...
        ├→ SniperStrategy (strategies/hft/sniper.py) ✅ 无pandas
        ├→ VultureStrategy (strategies/hft/vulture.py) ✅ 无pandas
...
| **SniperStrategy** | ✅ 可用 | ✅ 无pandas/numpy | HFT（5x杠杆，时间止损）|
| **VultureStrategy** | ✅ 可用 | ✅ 无pandas/numpy | HFT（5x杠杆，硬止损）|
| **DualEMAStrategy** | ✅ 可用 | ✅ 无pandas | 趋势（1.5x杠杆，移动止损）|
| **PullbackStrategy** | ⚠️ 已禁用 | ❌ 有pandas/numpy | 趋势（待重构）|

### 16.5 关键修复和优化（2026年1月13日）
