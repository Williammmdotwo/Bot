...
*项目目前运行在新加坡服务器1G1CPU*
*最后更新：2026年1月20日 - 高优先级修复：OrderManager资金检查、PositionManager指数退避、BaseStrategy市价单逻辑*
...
**核心职责**：
- 接收策略下单请求
- 风控检查（集成PreTradeCheck + CapitalCommander资金检查）
- 调用Gateway发单
- 追踪订单状态
- 自动撤单

**🔧 关键修复（2026-01-20）**：
- **资金检查集成**：在调用Gateway前调用CapitalCommander.check_buying_power()，避免资金不足订单被交易所拒绝
- **平仓检测**：支持通过symbol+side判断平仓操作，平仓时不检查资金
- **异常处理**：资金检查失败时记录警告但继续下单（向后兼容）
...
**修复效果示例**：
```
修复前：下单失败: BTC-USDT-SWAP buy 0.1 (被交易所拒绝)
修复后：🚫 资金检查未通过 [scalper_v1]: BTC-USDT-SWAP buy 0.1000, amount=5000.00 USDT
         (直接在本地拦截，不发送到交易所)
```
...
**🔧 关键修复（2026-01-20）**：
- **指数退避机制**：API持续报错时重试间隔从1秒递增至60秒（1s→2s→4s→8s→16s→32s→60s）
- **熔断保护**：连续失败10次后触发熔断，暂停60秒后重试，避免无限循环和日志爆炸
- **自动恢复**：成功时重置计数器和退避时间，正常时保持30秒间隔
...
**修复效果示例**：
```
修复前：定时持仓同步任务异常: Connection refused (无限循环，日志爆炸)
修复后：⚠️ [同步失败 3/10] 等待 4.0 秒后重试。错误: Connection refused
        🚨 [熔断触发] 持仓同步连续失败 10 次，暂停 60 秒后重试
```
...
**🔧 关键修复（2026-01-20）**：
- **市价单止损逻辑优化**：市价单（紧急平仓）允许止损价为0，但不发送止损单
- **限价单保护**：限价单必须提供有效止损价（>0），否则拒绝下单
- **向后兼容**：保持原有限价单逻辑不变，仅优化市价单处理
...
**修复效果示例**：
```
修复前：策略 scalper_v1 止损价格无效: stop=0.00 (非市价单必须提供止损价)
        (市价平仓被拒绝)
修复后：策略 scalper_v1 市价单止损价无效 (stop=0.0)，将只发送主订单，不发送止损单
        策略 scalper_v1 下单成功: BTC-USDT-SWAP sell 0.1000 @ 23456.78, stop=0.00 (市价)
```
...
- **Private WebSocket 非阻塞连接**：发送登录包后立即启动消息循环，不等待确认
- **Private WebSocket URL 修复**：使用标准模拟盘 URL，避免订阅失败
- **杠杆配置优化**：优先从策略配置中读取杠杆值，支持自定义杠杆设置
- **心跳响应拦截**：自动拦截 "pong" 响应，避免 JSON 解析错误，提升 WebSocket 稳定性
- **撤单容错处理**：捕获错误码 1 和 51402，防止策略因订单不存在而卡死
...
**日志级别优化**：
- **普通风控拦截**（金额、频率、单一币种敞口）：DEBUG级别，减少日志噪音
- **严重风险警报**（全局杠杆超限）：WARNING级别，保持风险提醒
...
- **日志级别优化**: PreTradeCheck + OrderManager
  - 普通风控拦截（金额、频率、单一币种敞口）：DEBUG级别
  - 严重风险警报（全局杠杆超限）：WARNING级别
  - 减少日志噪音，保留关键风险提醒
...
1. **OrderManager 资金检查缺失** ✅
   - 添加 CapitalCommander 依赖注入
   - 在 submit_order 前检查资金，避免订单被交易所拒绝
   - 支持平仓检测（symbol + side），平仓时不检查资金
   - 异常处理：资金检查失败时记录警告但继续下单（向后兼容）

2. **PositionManager 同步循环死锁** ✅
   - 添加指数退避机制（1s → 60s）
   - 添加熔断保护（连续失败10次触发）
   - 自动恢复机制（成功时重置计数器）
   - 避免日志爆炸和无限循环

3. **BaseStrategy 市价单止损逻辑错误** ✅
   - 优化止损价判断逻辑
   - 市价单允许止损价=0，但不发送止损单
...
**最后更新**: 关键修复 - OrderManager资金检查、PositionManager指数退避、BaseStrategy市价单逻辑
