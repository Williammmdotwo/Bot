...
def main_strategy_loop(data_manager, symbol="BTC-USDT", use_demo=False, postgres_db=None):
    ...
    try:
        ...
        if postgres_db:
            try:
                postgres_db.insert_ai_decision({
                    ...
                    "output_signal": parsed_signal,
                    "analysis_type": "technical_analysis",
                    "timestamp": int(time.time())
                })
            except Exception as db_error:
                logger.warning(f"Failed to store decision in database: {db_error}")
...
def _format_indicators_for_prompt(indicators: Dict) -> str:
    """Format technical indicators for prompt display"""
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            initialize_dependencies(data_handler, None)  # No AI client needed
...
def _format_orderbook_for_prompt(orderbook: Dict) -> str:
    """Format orderbook data for prompt display"""
    if not orderbook:
        return "订单簿数据不可用"
...
def _format_volume_profile_for_prompt(volume_profile: Dict) -> str:
    """Format volume profile for prompt display"""
...
def _format_sentiment_for_prompt(sentiment: Dict) -> str:
    """Format market sentiment for prompt display"""
    if not sentiment:
        return "市场情绪数据不可用"
...
def _format_historical_trends_for_prompt(historical_data: Dict) -> str:
    """
    格式化历史趋势分析用于提示词
    ...
    try:
        ...
        logger.error(f"Failed to format historical trends for prompt: {e}")
        return "历史趋势: 分析失败"
