...
from src.strategy_engine.core import merge_historical_with_current
...
# 兼容性别名（保持测试代码不变）
_format_indicators_for_prompt = format_indicators_for_display
_format_orderbook_for_prompt = format_orderbook_for_display
_format_volume_profile_for_prompt = format_volume_profile_for_display
_format_sentiment_for_prompt = format_sentiment_for_display
_merge_historical_with_current = merge_historical_with_current
_format_historical_trends_for_prompt = format_historical_trends_for_display
_analyze_trend_consistency = analyze_trend_consistency
_identify_key_turning_points = identify_key_turning_points
_analyze_volatility_across_timeframes = analyze_volatility_across_timeframes
...
class TestFormatHistoricalTrendsForPrompt:
    ...
    def test_format_historical_trends_complete(self):
        ...
        with patch('src.strategy_engine.formatters.market_data_formatter.analyze_trend_consistency') as mock_consistency:
            mock_consistency.return_value = {"overall_consistency": "高度一致"}

            with patch('src.strategy_engine.formatters.market_data_formatter.identify_key_turning_points') as mock_turning:
                mock_turning.return_value = ["关键转折点1", "关键转折点2"]

                with patch('src.strategy_engine.formatters.market_data_formatter.analyze_volatility_across_timeframes') as mock_volatility:
...
class TestAnalyzeVolatilityAcrossTimeframes:
    """analyze_volatility_across_timeframes函数测试"""
